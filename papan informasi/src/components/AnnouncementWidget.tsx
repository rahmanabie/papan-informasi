  ...
  return (
    <div className="rounded-xl shadow-lg overflow-hidden h-full flex flex-col text-sm">
      ...
      <div className="flex-1 relative overflow-hidden">
        {showEditForm ? (
          <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 z-20">
            {editingAnnouncement && (
              <form onSubmit={handleSaveAnnouncement} className="w-full max-w-4xl bg-white rounded-lg shadow-lg text-sm">
                <div className="p-3 bg-gray-50 border-b flex justify-between items-center">
                  <h3 className="text-base font-semibold text-gray-800">Edit Agenda Kegiatan</h3>
                  <button 
                    type="button"
                    onClick={() => {
                      setEditingAnnouncement(null);
                ...
                <div className="flex border-b bg-gray-50">
                  <button
                    type="button"
                    className={`px-4 py-2 ${activeTab === 'basic' ? 'bg-white border-b-2 border-green-500 text-green-600' : 'text-gray-600 hover:text-gray-800'}`}
                  ...
                  <button
                    type="button"
                    className={`px-4 py-2 ${activeTab === 'advanced' ? 'bg-white border-b-2 border-green-500 text-green-600' : 'text-gray-600 hover:text-gray-800'}`}
                  ...
                  <button
                    type="button"
                    className={`px-4 py-2 ${activeTab === 'participants' ? 'bg-white border-b-2 border-green-500 text-green-600' : 'text-gray-600 hover:text-gray-800'}`}
                ...
                <div className="border-t p-4 flex justify-between items-center bg-gray-50">
                  ...
                  <div className="flex space-x-2">
                    ...
                    className={`flex items-center px-1.5 py-0.5 text-xs ${inlineEditMode ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-gray-500 hover:bg-gray-600'} text-white rounded transition`}
                    title="Mode Edit"
                  >
                    <Edit className="w-3 h-3 mr-0.5" /> {inlineEditMode ? 'Selesai' : 'Edit'}
                  </button>
                  <button
                    onClick={handleAddAnnouncement}
                    className="flex items-center px-1.5 py-0.5 text-xs bg-green-500 text-white rounded hover:bg-green-600 transition"
                  ...
                  {selectedAnnouncementId && (
                    <button
                      ...
                      className="flex items-center px-1.5 py-0.5 text-xs bg-blue-500 text-white rounded hover:bg-blue-600 transition"
                      title="Edit Detail"
                    >
                      <Settings className="w-3 h-3 mr-0.5" /> Detail
                    </button>
                  )}
                </div>
              )}
            ...
            <div className="flex-1 overflow-hidden">
              ...
              >
                {announcements.map((announcement) => (
                  <div 
                    key={announcement.id}
                    className={`mb-4 cursor-pointer relative ${selectedAnnouncementId === announcement.id ? 'ring-1 ring-green-500' : ''}`}
